...
const categoryQueryMap: Record<string, string> = {
  'Pain Relief': 'pills analgesic paracetamol',
  'Antibiotics': 'antibiotics medicine pills',
  'Vitamins': 'vitamins supplements',
  'Cold & Flu': 'cough cold syrup',
  'Digestive': 'stomach digestion medicine',
  'Diabetes': 'insulin diabetes',
  'Heart': 'heart medication',
  'Skin Care': 'skin cream ointment topical',
};

function getImageUrl(med: MedicineItem) {
  if (med.imageUrl && !med.imageUrl.includes('placehold.co')) return med.imageUrl;
  const query = med.category && categoryQueryMap[med.category] ? categoryQueryMap[med.category] : med.medicineName || 'medicine';
...
const StaticMedicinesPage: FC = () => {
  return (
    <div className="container py-10">
      <h1 className="font-display font-bold text-2xl mb-6">Static Medicines (Provided List)</h1>
      <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {medicines.map((med, index) => (
          <div key={index} className="card border rounded-lg p-4 bg-card/50">
            <img src={getImageUrl(med)} alt={med.medicineName} className="w-full h-40 object-cover mb-3 rounded" />
            <h3 className="font-semibold text-lg">{med.medicineName} <span className="text-sm text-muted-foreground">({med.strength})</span></h3>
            <p className="text-sm"><strong>Brand:</strong> {med.brandName}</p>
            <p className="text-sm"><strong>Price:</strong> <span className="text-[1.1em]">৳</span>{med.price}</p>
...
export default StaticMedicinesPage;
